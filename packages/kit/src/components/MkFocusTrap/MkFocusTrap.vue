...
import { useEscapeKeydown } from '@magic/kit/hooks';
import { getEventCode, isString } from '@magic/kit/utils';
...
import type { FocusableLayer, FocusStart } from './types';
import {
...
export default {
  ...
  props: {
    loop: Boolean,
    trapped: Boolean,
    focusTrapEl: { type: Object as PropType<HTMLElement | null>, default: null },
    focusStartEl: {
      type: [Object, String] as PropType<HTMLElement | FocusStart>,
      default: 'first',
    },
  },
  emits: [
  ...
  setup(props, { emit }) {
    ...
    let lastFocusBeforeTrapped: HTMLElement | Element | null = null;
    let lastFocusAfterTrapped: HTMLElement | Element | null = null;
    ...
    const focusLayer: FocusableLayer = {
      paused: false,
      pause() {
        this.paused = true;
      },
    ...
    const onFocusIn = (e: FocusEvent) => {
      ...
      if (props.trapped) {
        if (isFocusedInTrap) {
          ...
          tryFocus(lastFocusAfterTrapped as HTMLElement, true);
        }
      }
    };
    ...
    async function startTrap() {
      ...
      if (trapContainer) {
        ...
        const isPrevFocusContained = trapContainer.contains(prevFocusedElement as Node);
    ...
    function stopTrap() {
      ...
      if (trapContainer) {
        ...
        if (
          ...
          tryFocus((lastFocusBeforeTrapped ?? document.body) as HTMLElement);
        }
